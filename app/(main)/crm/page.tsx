...
export default function CRMPage() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 52px - 24px)' }}>
      ...
      {bookClientOpen && selected && (
        <BookClientModal
          lead={selected}
          onClose={() => setBookClientOpen(false)}
          onBooked={(clientId) => {
...
type BookPath = 'new' | 'existing' | 'label'

function BookClientModal({ lead, onClose, onBooked }: {
  lead: Lead
  onClose: () => void
  onBooked: (clientId: string) => void
}) {
  const [path, setPath] = useState<BookPath | null>(null)
  ...
  const [existResults, setExistResults] = useState<Client[]>([])
  const [existSelected, setExistSelected] = useState<Client | null>(null)
  const [existSearching, setExistSearching] = useState(false)

  // Path C — Label booking
  const [labelSearch, setLabelSearch] = useState('')
  const [labelResults, setLabelResults] = useState<Client[]>([])
  const [labelSelected, setLabelSelected] = useState<Client | null>(null)
  ...
  // Path B — debounced search
  const existDebounce = useRef<ReturnType<typeof setTimeout>>()
  useEffect(() => {
    if (path !== 'existing') return
    const q = existSearch.trim()
    if (q.length < 2) { setExistResults([]); return }
    clearTimeout(existDebounce.current)
    setExistSearching(true)
    existDebounce.current = setTimeout(async () => {
      const { data } = await supabase.from('clients').select('id, type, name, fname, lname, email, phone')
        .or(`name.ilike.%${q}%,fname.ilike.%${q}%,lname.ilike.%${q}%,email.ilike.%${q}%`)
        .limit(10)
      setExistResults((data as Client[]) || [])
      setExistSearching(false)
    }, 250)
    return () => clearTimeout(existDebounce.current)
  }, [existSearch, path])

  // Path C — debounced label search
  const labelDebounce = useRef<ReturnType<typeof setTimeout>>()
  useEffect(() => {
    if (path !== 'label') return
    const q = labelSearch.trim()
    if (q.length < 2) { setLabelResults([]); return }
    clearTimeout(labelDebounce.current)
    setLabelSearching(true)
    labelDebounce.current = setTimeout(async () => {
      const { data } = await supabase.from('clients').select('id, type, name, artists')
        .eq('type', 'label').ilike('name', `%${q}%`).limit(10)
      setLabelResults((data as Client[]) || [])
  ...
  async function copyLink() {
    if (!tokenUrl) return
    try { await navigator.clipboard.writeText(tokenUrl) } catch (_) {}
    setCopied(true)
    setTimeout(() => setCopied(false), 2000)
  }
  ...
  async function submitExisting() {
    ...
    setSubmitting(true)
    await supabase.from('leads').update({ client_id: existSelected.id }).eq('id', lead.id)
    onBooked(existSelected.id)
    setSubmitting(false)
  }

  async function submitLabel() {
    if (!labelSelected || !orderedBy.trim() || !artist.trim()) return
    setSubmitting(true)
    await supabase.from('leads').update({ client_id: labelSelected.id }).eq('id', lead.id)
    onBooked(labelSelected.id)
    setSubmitting(false)
  }
  ...
  const leadName = [lead.fname, lead.lname].filter(Boolean).join(' ')
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div onClick={e => e.stopPropagation()} style={{ width: 480, maxHeight: '88vh', overflowY: 'auto', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12 }}>
        ...
        <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', position: 'sticky', top: 0, background: 'var(--surface)', zIndex: 1 }}>
          <div>
            <div style={{ fontFamily: 'Syne', fontWeight: 800, fontSize: 15 }}>Book Client</div>
            {leadName && (
              <div style={{ fontSize: 10, color: 'var(--text3)', marginTop: 2, fontFamily: 'DM Mono' }}>
                {leadName}{lead.email ? ` · ${lead.email}` : ''}
              </div>
            )}
          </div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: 'var(--text3)', fontSize: 18, cursor: 'pointer', lineHeight: 1 }}>×</button>
        ...
        <div style={{ padding: '8px 20px 0', display: 'flex', gap: 4, alignItems: 'center' }}>
          ...
          <div style={{ width: 6, height: 6, borderRadius: '50%', background: path ? 'var(--accent)' : 'var(--border)' }} />
          <span style={{ fontSize: 9, color: 'var(--text3)', fontFamily: 'DM Mono', marginLeft: 4 }}>
            {path ? 'Step 2 of 2' : 'Step 1 of 2'}
          </span>
        </div>
        ...
        <div style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          {!path && (
            <>
              <div style={{ fontSize: 12, color: 'var(--text2)', fontFamily: 'DM Mono', marginBottom: 4 }}>Who is this booking for?</div>
              {([
                { id: 'new', label: 'New client', desc: 'Send a registration form link' },
                { id: 'existing', label: 'Existing client', desc: 'Search and link to an existing record' },
                { id: 'label', label: 'Label booking', desc: 'Pick a label, contact, and artist' },
              ] as { id: BookPath, label: string, desc: string }[]).map(opt => (
                <button
                  key={opt.id}
                ...
                >
                  <span style={{ fontSize: 12, fontFamily: 'Syne', fontWeight: 700, color: 'var(--text)', letterSpacing: '0.04em' }}>{opt.label}</span>
                  <span style={{ fontSize: 11, fontFamily: 'DM Mono', color: 'var(--text3)' }}>{opt.desc}</span>
                </button>
              ))}
            </>
          )}
          ...
          {path === 'new' && !tokenUrl && (
            <>
              <div style={{ fontSize: 12, color: 'var(--text2)', fontFamily: 'DM Mono', lineHeight: 1.6 }}>
                We'll generate a registration link to send to{' '}
                {lead.email ? <strong style={{ color: 'var(--text)' }}>{lead.email}</strong> : 'the client'}.
                The link expires in <strong style={{ color: 'var(--text)' }}>7 days</strong>.
              </div>
              <div style={{ fontSize: 10, color: 'var(--text3)', fontFamily: 'DM Mono', background: 'var(--surface2)', padding: '8px 10px', borderRadius: 6, lineHeight: 1.6 }}>
                When the client completes the form, their profile will be created automatically and this lead will be linked.
              </div>
            </>
          )}
          ...
          {path === 'new' && tokenUrl && (
            <>
              <div style={{ fontSize: 11, color: 'var(--text3)', fontFamily: 'DM Mono', marginBottom: 2 }}>Registration link</div>
              <div style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '8px 12px', fontFamily: 'DM Mono', fontSize: 11, color: 'var(--accent)', wordBreak: 'break-all', lineHeight: 1.6 }}>
                {tokenUrl}
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={copyLink} style={{ flex: 1, padding: '8px 0', background: copied ? 'rgba(78,240,162,0.15)' : 'var(--surface2)', color: copied ? 'var(--booked)' : 'var(--text2)', border: `1px solid ${copied ? 'var(--booked)' : 'var(--border)'}`, borderRadius: 6, fontFamily: 'Syne', fontWeight: 700, fontSize: 10, cursor: 'pointer', letterSpacing: '0.06em', textTransform: 'uppercase', transition: 'all 0.15s' }}>
                  {copied ? '✓ Copied!' : 'Copy Link'}
                </button>
                <button onClick={emailLink} style={{ flex: 1, padding: '8px 0', background: 'var(--surface2)', color: lead.email ? 'var(--text2)' : 'var(--text3)', border: '1px solid var(--border)', borderRadius: 6, fontFamily: 'Syne', fontWeight: 700, fontSize: 10, cursor: lead.email ? 'pointer' : 'not-allowed', letterSpacing: '0.06em', textTransform: 'uppercase' }}>
                  ✉ Email Link
                </button>
              </div>
            </>
          ...
          {/* ── Step 2B: Existing client ── */}
          {path === 'existing' && (
            <>
              {!existSelected ? (
                <>
                  <div>
                    <label style={labelS}>Search clients</label>
                    <input
                      autoFocus
                      value={existSearch}
                      onChange={e => { setExistSearch(e.target.value); setExistSelected(null) }}
                      placeholder="Name or email…"
                      style={inputS}
                    />
                  </div>
                  {existSearching && <div style={{ fontSize: 11, color: 'var(--text3)', fontFamily: 'DM Mono' }}>Searching…</div>}
                  {existResults.length > 0 && (
                    <div style={{ border: '1px solid var(--border)', borderRadius: 6, overflow: 'hidden' }}>
                      {existResults.map(c => (
                        <div
                          key={c.id}
                          onClick={() => setExistSelected(c)}
                        ...
                        >
                          <div>
                            <div style={{ fontSize: 12, fontFamily: 'DM Mono', color: 'var(--text)' }}>
                          ...
                          <span style={{ fontSize: 9, padding: '2px 7px', borderRadius: 10, background: c.type === 'label' ? 'rgba(200,240,78,0.15)' : 'rgba(139,144,168,0.12)', color: c.type === 'label' ? 'var(--accent)' : 'var(--text3)', fontFamily: 'Syne', fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase' }}>
                            {c.type === 'label' ? 'Label' : 'COD'}
                          </span>
                        </div>
                      ))}
                    </div>
                  )}
                  {existSearch.length >= 2 && !existSearching && existResults.length === 0 && (
                    <div style={{ fontSize: 11, color: 'var(--text3)', fontFamily: 'DM Mono', textAlign: 'center', padding: '12px 0' }}>No clients found</div>
                  )}
                </>
              ) : (
                <>
                  <div style={{ fontSize: 11, color: 'var(--text3)', fontFamily: 'DM Mono', marginBottom: 4 }}>Selected client</div>
                  <div style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '12px 14px' }}>
                    <div style={{ fontSize: 13, fontFamily: 'Syne', fontWeight: 700, color: 'var(--text)', marginBottom: 4 }}>
                      {existSelected.name || `${existSelected.fname || ''} ${existSelected.lname || ''}`.trim()}
                    </div>
                    <div style={{ fontSize: 11, color: 'var(--text3)', fontFamily: 'DM Mono', display: 'flex', gap: 16 }}>
                      {existSelected.email && <span>{existSelected.email}</span>}
                      {existSelected.phone && <span>{existSelected.phone}</span>}
                    </div>
                  </div>
                  <button onClick={() => setExistSelected(null)} style={{ alignSelf: 'flex-start', background: 'none', border: 'none', padding: 0, fontSize: 10, color: 'var(--text3)', fontFamily: 'DM Mono', cursor: 'pointer', textDecoration: 'underline' }}>
          ...
          {/* ── Step 2C: Label booking ── */}
          {path === 'label' && (
            <>
              {!labelSelected ? (
                <>
                  <div>
                    <label style={labelS}>Search labels</label>
                    <input
                      autoFocus
                      value={labelSearch}
                  ...
                  {labelSearching && <div style={{ fontSize: 11, color: 'var(--text3)', fontFamily: 'DM Mono' }}>Searching…</div>}
                  {labelResults.length > 0 && (
                    <div style={{ border: '1px solid var(--border)', borderRadius: 6, overflow: 'hidden' }}>
                      {labelResults.map(c => (
                        <div
                          key={c.id}
                          onClick={() => setLabelSelected(c)}
                          style={{ padding: '10px 14px', cursor: 'pointer', borderBottom: '1px solid var(--border)' }}
                          onMouseOver={e => (e.currentTarget.style.background = 'var(--surface2)')}
                          onMouseOut={e => (e.currentTarget.style.background = 'transparent')}
                        >
                          <div style={{ fontSize: 12, fontFamily: 'DM Mono', color: 'var(--text)' }}>{c.name}</div>
                        </div>
                      ))}
                    </div>
                  )}
                  {labelSearch.length >= 2 && !labelSearching && labelResults.length === 0 && (
                    <div style={{ fontSize: 11, color: 'var(--text3)', fontFamily: 'DM Mono', textAlign: 'center', padding: '12px 0' }}>No labels found</div>
                  )}
                </>
              ) : (
                <>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                    <div style={{ fontSize: 13, fontFamily: 'Syne', fontWeight: 700, color: 'var(--accent)' }}>{labelSelected.name}</div>
                    <button onClick={() => { setLabelSelected(null); setOrderedBy(''); setArtist('') }} style={{ background: 'none', border: 'none', padding: 0, fontSize: 10, color: 'var(--text3)', fontFamily: 'DM Mono', cursor: 'pointer', textDecoration: 'underline' }}>Change</button>
                  </div>
                  <div>
                    <label style={labelS}>Ordered by (A&amp;R / rep)</label>
                    <ContactPicker
                      clientId={labelSelected.id}
                      value={orderedBy}
                  ...
                  {(!orderedBy.trim() || !artist.trim()) && (
                    <div style={{ fontSize: 10, color: 'var(--text3)', fontFamily: 'DM Mono' }}>
                      Both fields required to submit.
                    </div>
                  )}
                </>
              )}
        ...
        <div style={{ padding: '12px 20px 20px', display: 'flex', gap: 8, position: 'sticky', bottom: 0, background: 'var(--surface)', borderTop: '1px solid var(--border)' }}>
          {path ? (
            <button onClick={resetPath} style={{ padding: '9px 16px', background: 'transparent', border: '1px solid var(--border)', color: 'var(--text2)', borderRadius: 6, fontFamily: 'DM Mono', fontSize: 11, cursor: 'pointer' }}>
              ← Back
            </button>
          ) : (
            <button onClick={onClose} style={{ padding: '9px 16px', background: 'transparent', border: '1px solid var(--border)', color: 'var(--text2)', borderRadius: 6, fontFamily: 'DM Mono', fontSize: 11, cursor: 'pointer' }}>
              Cancel
            </button>
          )}
          ...
          {path === 'new' && !tokenUrl && (
            <button
              ...
              disabled={generating}
              style={{ flex: 1, padding: '9px 0', background: generating ? 'var(--surface2)' : 'var(--accent)', color: generating ? 'var(--text3)' : '#0d0f14', border: 'none', borderRadius: 6, fontFamily: 'Syne', fontWeight: 700, fontSize: 11, cursor: generating ? 'not-allowed' : 'pointer', letterSpacing: '0.05em', textTransform: 'uppercase' }}>
          ...
          {path === 'new' && tokenUrl && (
            <button
              onClick={onClose}
              style={{ flex: 1, padding: '9px 0', background: 'var(--surface2)', color: 'var(--text2)', border: '1px solid var(--border)', borderRadius: 6, fontFamily: 'Syne', fontWeight: 700, fontSize: 11, cursor: 'pointer', letterSpacing: '0.05em', textTransform: 'uppercase' }}>
              Done
            </button>
          )}

          {/* Path B footer */}
          {path === 'existing' && existSelected && (
            <button
              onClick={submitExisting}
              disabled={submitting}
              style={{ flex: 1, padding: '9px 0', background: submitting ? 'var(--surface2)' : 'var(--accent)', color: submitting ? 'var(--text3)' : '#0d0f14', border: 'none', borderRadius: 6, fontFamily: 'Syne', fontWeight: 700, fontSize: 11, cursor: submitting ? 'not-allowed' : 'pointer', letterSpacing: '0.05em', textTransform: 'uppercase' }}>
              {submitting ? 'Linking…' : 'Link Client →'}
            </button>
          )}

          {/* Path C footer */}
          {path === 'label' && labelSelected && (
            <button
              onClick={submitLabel}
              disabled={!orderedBy.trim() || !artist.trim() || submitting}
              style={{ flex: 1, padding: '9px 0', background: (!orderedBy.trim() || !artist.trim() || submitting) ? 'var(--surface2)' : 'var(--accent)', color: (!orderedBy.trim() || !artist.trim() || submitting) ? 'var(--text3)' : '#0d0f14', border: 'none', borderRadius: 6, fontFamily: 'Syne', fontWeight: 700, fontSize: 11, cursor: (!orderedBy.trim() || !artist.trim() || submitting) ? 'not-allowed' : 'pointer', letterSpacing: '0.05em', textTransform: 'uppercase' }}>
