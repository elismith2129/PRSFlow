...
// ─────────────────────────────────────────────────────────────────────────────
// FLO'S 8:50 BRIEFING (Eli, 2026-09-07): reads the night's notes, the flags,
// the week's holds, the duty record and the open tasks, and writes ONE
// briefing row (flo_briefings) for the operational day. Runs from Vercel cron
// at 8:50 AM Los Angeles (two UTC schedules cover PDT/PST — the second run
// sees the row exists and does nothing).
//
// THE VOICE RULING: just the facts. The prompt below is Flo's personality —
// edit it there, not by post-processing.
//
// THE AUDIENCE RULING: `shared` is for everyone; `slices` are per-seat
// accountability (each person sees their own; owners see all). The route
// EXCLUDES private tasks from the model's input entirely — a briefing row is
// readable by all authenticated staff, so nothing private may enter it.
//
...
export async function GET(request: Request) {
  ...
  // Idempotent: the DST double-schedule (and any manual re-trigger) must not
  // overwrite a briefing people already read. ?force=1 regenerates.
  const existing = await db.from('flo_briefings').select('id').eq('date', today).limit(1)
  if (existing.data?.length && !force) {
    return NextResponse.json({ ok: true, skipped: 'already written', date: today })
  }
...
- Dates in the data are the studio's operational days (the day rolls at 8:50 AM — overnight notes belong to the night before).`,
    ...
    await db.from('app_errors').insert({
      message: `Flo briefing cron: ${err instanceof Error ? err.message : String(err)}`.slice(0, 1000),
      url: '/api/cron/flo-briefing',
      meta: { source: 'flo-briefing-cron' },
