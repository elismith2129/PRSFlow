...
--   Section 6a checklist-photos bucket → private + authenticated-only storage policies
--   Section 7  drop anon policies on the expenses / client-ids buckets
--
-- Tier predicates:
...
--
-- RESILIENCE: every public-schema table block is wrapped in a
--   DO $$ BEGIN IF EXISTS (SELECT FROM pg_tables ...) THEN ... END IF; END $$;
-- guard, so a table that does not exist in the live DB is skipped cleanly
-- instead of aborting the migration. (The audit was doc-based; some documented
-- tables — e.g. expense_rows — are not actually present.)
--
...
-- (user_profiles is a hard dependency for the helpers below and is confirmed
--  to exist, so this section is not guarded.)
-- ===========================================================================
...
-- Each drop is guarded so a missing table does not abort the migration.
-- ===========================================================================
...
DO $$ BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'work_orders') THEN
    DROP POLICY IF EXISTS "Public access" ON work_orders;
  END IF;
END $$;

DO $$ BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'qc_reports') THEN
    DROP POLICY IF EXISTS "Public access" ON qc_reports;
  END IF;
END $$;

DO $$ BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'contact_log') THEN
...
DO $$ BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'lead_activity') THEN
...
DO $$ BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'registration_tokens') THEN
...
DO $$ BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'studio_time_rows') THEN
...
DO $$ BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'rental_rows') THEN
...
DO $$ BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'payment_rows') THEN
...
DO $$ BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'expense_rows') THEN
...
DO $$ BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'equipment_condition_rows') THEN
...
DO $$ BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'equipment_condition_notes') THEN
...
DO $$ BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'daily_ops_submissions') THEN
...
DO $$ BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'checklists') THEN
...
DO $$ BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'petty_cash_entries') THEN
...
DO $$ BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'petty_cash_balances') THEN
...
DO $$ BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'stock_items') THEN
    ALTER TABLE stock_items ENABLE ROW LEVEL SECURITY;

    DROP POLICY IF EXISTS stock_items_sel ON stock_items;
    CREATE POLICY stock_items_sel ON stock_items FOR SELECT TO authenticated
      USING (get_my_role() IN ('asst_manager','billing','manager','owner','runner','tech'));

    DROP POLICY IF EXISTS stock_items_ins ON stock_items;
    CREATE POLICY stock_items_ins ON stock_items FOR INSERT TO authenticated
      WITH CHECK (get_my_role() IN ('asst_manager','billing','manager','owner','runner','tech'));

    DROP POLICY IF EXISTS stock_items_upd ON stock_items;
    CREATE POLICY stock_items_upd ON stock_items FOR UPDATE TO authenticated
      USING (get_my_role() IN ('asst_manager','billing','manager','owner','runner','tech'))
      WITH CHECK (get_my_role() IN ('asst_manager','billing','manager','owner','runner','tech'));

    DROP POLICY IF EXISTS stock_items_del ON stock_items;
    CREATE POLICY stock_items_del ON stock_items FOR DELETE TO authenticated
      USING (get_my_role() IN ('manager','billing','owner'));
  END IF;
END $$;
...
DO $$ BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'mics') THEN
...
DO $$ BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'mic_checkins') THEN
...
DO $$ BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'mic_inventory_quantities') THEN
...
DO $$ BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'mic_inventory_submissions') THEN
...
DO $$ BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'mic_inventory') THEN
    ALTER TABLE mic_inventory ENABLE ROW LEVEL SECURITY;

    DROP POLICY IF EXISTS mic_inventory_sel ON mic_inventory;
    CREATE POLICY mic_inventory_sel ON mic_inventory FOR SELECT TO authenticated
      USING (get_my_role() IN ('asst_manager','billing','manager','owner'));

    DROP POLICY IF EXISTS mic_inventory_ins ON mic_inventory;
    CREATE POLICY mic_inventory_ins ON mic_inventory FOR INSERT TO authenticated
      WITH CHECK (get_my_role() = 'owner');

    DROP POLICY IF EXISTS mic_inventory_upd ON mic_inventory;
    CREATE POLICY mic_inventory_upd ON mic_inventory FOR UPDATE TO authenticated
      USING (get_my_role() = 'owner')
      WITH CHECK (get_my_role() = 'owner');

    DROP POLICY IF EXISTS mic_inventory_del ON mic_inventory;
    CREATE POLICY mic_inventory_del ON mic_inventory FOR DELETE TO authenticated
      USING (get_my_role() = 'owner');
  END IF;
END $$;
...
DECLARE r record;
BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'dashboard_tasks') THEN
...
DECLARE r record;
BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'dashboard_task_comments') THEN
...
DO $$ BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'flag_comments') THEN
...
DO $$ BEGIN
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'srs_log') THEN
...
-- (storage.objects / storage.buckets are Supabase system tables — always exist,
--  so no existence guard is needed here.)
-- ===========================================================================

-- Make the bucket private (no more public object URLs). No-op if the bucket
-- does not exist (0 rows updated).
...
DROP POLICY IF EXISTS "checklist_photos_select" ON storage.objects;
