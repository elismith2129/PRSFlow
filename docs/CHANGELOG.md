...
the WO and in the PDF"; then "change it to 45%, and make it adjustable on the WO."
Migration `20260917120000`: `work_orders.food_fee_pct text` (whole percent; NULL = default).
`FOOD_SERVICE_FEE_PCT = 45`, `foodFeePct(raw)` and `foodServiceFee(receipts, pct)` live in
...
[45]% = Billed** — the percent is a small input on the WO, blank means the default. The
package's Food Budget page prints Receipts / Service fee (N%) / **Total billed**, then
Budget / Remaining. The budget is still compared against receipts — that's what the runners
were given to spend; the fee is what the label pays.
...
should change, it's a ruling, not a bug. The fee percent rides the atomic WO save like every
other WO field; `/api/wo-package` selects `*`, so the PDF sees it.
...
v1.18.0 **is merged to `main`** — that entry's "branch `mic-sheet` pending"
caveat no longer applies.

**Calendar history import.** `scripts/importCalendar2026.mjs` rebuilds the year
from the WordPress export. It **snapshots then DELETES all `bookings` rows**
before inserting (approved — everything there was test data).
`bookings.imported_at`: NULL = born in PRSFlo, non-NULL = imported.

**Money.**
- `'lockout'` `BookingStatus` — rent-only monthlies. Occupied and payable,
  nothing daily. Gets a WO (rent is invoiced), reads booked-green.
- **Monthly split**: a flat monthly ($19.5k/$29.5k) allocated across dated
  studio rows by largest-remainder, summing to the cent. Rows then behave as
  ordinary day-rate rows. OT typed per deal.
- **`DAY_HOUR_RATIO = 10`** (`lib/woTotals.ts`) — day rate is always 10×
  hourly; day-row OT = day ÷ 10. Twin-field sync on edit and batch.
- **3% card surcharge** on COD WOs (`payment_rows.fee_amount`): `amount` is
  what hit the card, `fee_amount` the slice inside it, derived via
  `cardFeeOfCharged`. Added to the grand total, so a card payment moves the
  balance by exactly its base. WO shows "If paying by card (incl. 3%)".

**Notes.**
- **Shift notes v4** → `shift_note_docs`: one large autosaving field per runner
  per shift, opener/floater/closer chips, ~1s debounce + flush on close + draft
  net, 8:50 seal kept. Replaces the never-adopted `shift_log_entries`.
- **`components/shared/RichNote.tsx`** — the app's only rich-text surface: bold,
  bullets, tab indent. contentEditable + `execCommand`, no dependency.
  Whitelist `sanitizeNote()` at edit-sync AND display; paste forced to plain
  text; legacy plain-text notes render unchanged, **no data migration**.
- **`runner_section_notes`** — one shared general-notes box per (studio,
  operational day, section: stock|office|mics), surfaced in Daily Ops.
- **`myday_note_drafts`** — the manager composer autosaves, **one row per
  author** (not per date), private to the author, cleared on submit;
  `editing_post_id` persisted so a resumed edit updates rather than duplicates.
  800ms debounce + flush on unmount/`visibilitychange`/`pagehide`.

**The operational day.** `opsToday()` (= `shiftLogDate()`, 8:50 AM LA) is the
day key for every runner surface: sessions and WOs survive midnight, checklists
file under the night, the Wednesday office gate holds. **Punches deliberately
stay on the calendar day** (payroll counts midnights).

**Runner test pass (ARS).** Soundstar → Ameraycan; TEST MIC deactivated; bagel
flavor order in the par text; **Dairy & Creamers → Fridge** (butter, cream
cheese, cookie dough, ice moved in — the runner walks to one fridge); mic
check-in test data wiped; Odds typed quantities; Genelec qty via `mics.has_qty`;
`RunnerGuard` defers expiry while typing (the mid-stock logout); ARS + ERS
office stock lists seeded.

**Also.** Registration "Create profile" now actually writes
`clients.profile_confirmed_at` (badge cleared optimistically and returned on
refetch); `leasing` lead status (orchid, aliased to `--c-st-tech`, excluded
from needs-contact predicates); iPhone dashboard/calendar pass; WO popup crash
(`wo` dereferenced above its null guard in the card-fee block); calendar mobile
room-row height uses MAX `numLanes`; `dbResult` gained `{ silent }`.

**Migrations (all hand-run):** `20260826120000_ars_ers_stock.sql`,
`20260826130000_ars_ers_office_stock.sql`, `20260826140000_shift_note_docs.sql`,
`20260826150000_bookings_imported_at.sql`,
`20260826160000_payment_rows_fee_amount.sql`,
`20260828120000_ars_runner_feedback.sql`,
`20260828130000_runner_section_notes.sql`,
`20260831120000_myday_note_drafts.sql` (run Aug 31 — verified `to_regclass`
non-null; the first `select` came back NULL because the file hadn't been run
yet, which is the normal failure and not a code fault).

**WATCH-OUT #1 — `imported_at` is provenance, not state.** Promotion of a
future-dated imported booking must NOT clear it. Past + imported = read-only
history everywhere (no WO, no invoice number, no daily ops, no runner hub);
today-or-later + imported = a legacy WO-less block the promotion path adopts.
This is enforced in the UI, not RLS, precisely because RLS can't allow
promotion and forbid editing at once. Imported rows are also excluded from My
Day's missing-WO alarm — that alarm means "a WO failed to create", and forty
years of history is not an anomaly.

**WATCH-OUT #2 — a lockout is excluded BY STATUS.** Every operational surface
selects `status='confirmed'`, so `'lockout'` never matches and needs no
exclusion flag anywhere. Don't add one. And don't add `'lockout'` to
`NON_SESSION_STATUSES` — unlike Tour/Tech/Open-Hours it *must* get a WO.

**WATCH-OUT #3 — `opsToday()` vs `getLocalToday()`.** Runner surfaces use
`opsToday()`; admin/calendar surfaces use `getLocalToday()`. Using the wrong
one is invisible in daylight and wrong for nine hours a night. If the 8:50
boundary moves, `shiftLogDate()` **and** the `shift_note_docs` /
`shift_log_entries` RLS policies move together — the interval is hard-coded in
the SQL.

**WATCH-OUT #4 — `DAY_HOUR_RATIO` takes no exceptions.** Special deals are
typed after converting. The moment the constant grows an exception list it
stops being a law. Monthly OT is the one deliberate opt-out (typed per deal,
never autofilled).

**WATCH-OUT #5 — `dbResult(label, error, { silent: true })` suppresses the
TOAST ONLY.** The `app_errors` report and the false return are unchanged, and
a silent caller MUST surface failure in its own UI (the composer lozenge does).
Silence in the interface is the defect class this helper exists to kill.

**WATCH-OUT #6 — cancel the debounce before deleting a draft.** On submit,
`myday_note_drafts` is deleted; if the pending autosave timer is still armed it
fires afterwards and resurrects the text that was just posted.

**Files:** `scripts/importCalendar2026.mjs`, `components/calendar/WorkOrderPopup.tsx`,
`lib/woTotals.ts`, `lib/woPdf.ts`, `lib/supabase.ts`, `lib/time.ts`, `lib/dailyOps.ts`,
`lib/myday.ts`, `lib/db.ts`, `lib/format.ts`, `lib/checklist-items.ts`,
`components/shared/RichNote.tsx`, `components/carved/index.tsx`,
`components/ui/StatusBadge.tsx`, `app/(main)/my-day/page.tsx`,
`app/(main)/calendar/page.tsx`, `app/(main)/page.tsx`, `app/(main)/daily-ops/page.tsx`,
`app/runner/[studio]/{page,stock,mics,shift-notes,checklist}`, `styles/globals.css`,
plus the eight migrations above.
...
- **Venue guard (WorkOrderPopup)**: saving now REFUSES (office only) when a
  dated studio row has no venue — a venue-less row projected a booking card
  the calendar cannot place (found in launch testing). Red banner + row
  highlight via the existing missing-times mechanism.
- **Day sheet on admin desktop** is a centered 620px card, not the phone
  bottom-sheet (which rendered full-bleed under the rail). Phones unchanged;
