...
| PWA | Web manifests + icons (no service worker) | Installable to home screen (`display: standalone`). Two manifests/icon sets — main app (`/manifest.json`, `start_url /`) and runner hub (`/runner-manifest.json`, `start_url /runner`). Wired via Next.js Metadata API. **No offline caching.** Icons generated by `scripts/generate-icons.js` (`sharp` dev dep) |
