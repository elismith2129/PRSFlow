...
| `app/api/auth/pin/route.ts` | PIN login route (service role) — DB per-IP lockout, `verify_staff_pin` RPC, mints a magic-link token for the browser's `verifyOtp`. |
