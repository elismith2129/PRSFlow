...
| `app/inquiry/page.tsx` | Public, unauthenticated lead-capture form (outside `(main)`, no AuthGuard). Inserts a `leads` row (`status:'uncontacted'`, `source:'Web Inquiry'`) via the anon key; in-place thank-you; studio-photo bg (`public/inquiry-bg.jpg`) + frosted card + `public/paramount-logo.png` |
...
| public-inquiry-form | **(`acd65f3`/`9857f6f`/`8e25593`)** `app/inquiry/page.tsx` — public (no AuthGuard, outside `(main)`) form (first/last/email/phone/notes) that inserts a `leads` row (`status:'uncontacted'`, `source:'Web Inquiry'`) via the browser anon key; in-place thank-you on submit (no redirect). Full-bleed studio photo (`public/inquiry-bg.jpg`) + dark gradient + frosted card + `public/paramount-logo.png` (PRSFlo wordmark dropped on this page). |
