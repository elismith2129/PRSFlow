...
- ✅ 4.6b — Two UX improvements to the registration form: (1) `capture="environment"` on the ID upload file input — offers rear camera on iOS Safari and Android Chrome, falls back to standard picker on desktop. (2) Scrollable T&Cs embed — 11-section Terms & Conditions panel (max-height 280px) appears above the acknowledgment checkbox; content lives in `lib/terms.ts`; sections use 12px DM Mono uppercase headings and 13px DM Mono body at line-height 1.65 with 12px between sections. `terms_accepted` boolean and submit logic unchanged.
