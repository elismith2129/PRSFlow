...
**v1.3.0 — runner table readability + staff reassignment + admin fill-down (same session).**

*Runner Studio Time table.* Type 9–10px → 11–12px, numbers moved off `--text2` to `--text`, all cells centred, and the per-cell 9px overrides that survived the first pass (Hrs, Notes button, initials pill, **the OT-hours input** — the smallest tap target in a table used one-handed) cleaned up. **Date + Studio frozen** via `position: sticky`. **Studio column added** — it never existed on the runner view, which is what made two rooms on one date read as a duplicate. **Zebra by DAY not by row**, since a day is 2–3 lines once staff sub-rows are in. **Today-only by default** with an `All N days` toggle (multi-day WOs only; falls back to all rows when today has none, so a WO opened a day early doesn't look broken). Filtering is render-only — `handleSaveChanges` iterates all of `stRows`, so edits to hidden days still save.
- **Bug I introduced and fixed same session:** sticky cells were given the row's *translucent* tint, so scrolling columns showed through the frozen ones. Sticky backgrounds must be opaque — now composited (`linear-gradient(tint, tint), var(--surface)`) so the tint still reads over a solid base.
- **`getLocalToday` consolidated into `lib/time.ts`** — it existed byte-identical in **five** files (both runner hubs, runner mics, `WorkOrderPopup`, `MicInventorySection`) and I needed a sixth. Timezone-sensitive date maths duplicated six ways is exactly the audit's flagged defect class; a drifted copy would show tomorrow's sessions after 5pm local. **The `ST_GRID` column template was also defined three times** in the runner table (header + studio rows + staff rows) — one constant now, so header and body can't drift out of alignment.

*Staff reassignment on the runner (Eli: clicking the pill only showed the name).* The pill popover gained a **Change** button opening a top-anchored sheet (centred, not pill-anchored — the pill sits mid-table with nowhere to open, and top-anchored keeps it clear of the keyboard): name field with a `<datalist>` of the row's pool, free text allowed, blank = unassign. **Two save scopes: "Save this day" and "Save all N {role} days"** — a 30-day run otherwise meant typing the same name thirty times. `'all'` targets rows sharing the edited row's **role**, so changing the engineer can't wipe the assistants. Roster comes from `useStaffPools()` (shared with the CRM lead picker, so the two can't offer different lists). **Written immediately, not queued** — `eng_name` is NOT part of `handleSaveChanges` (that only persists engineer times/hours/charge), so a queued edit would have been silently dropped; matches the date cell's existing behaviour. Draft seeds from the row's OWN `eng_name`, never the WO-level fallback, which would otherwise pin an inherited name onto the row. **`dbResult` imported into the runner WO page for the first time** — none of that page's writes were error-checked, a standing-rule gap; only the new write is wrapped, the retrofit is a separate sweep.

*Admin WO fill-down (Eli: same idea for start/end times, rates, studio — "we may change it, happens often").* `applyFieldToAllDays(sourceId, keys)` copies one field from one row onto every other **day**, routed through **`updateStRow`** so charge/OT hours/OT rate/OT charge/engineer charge all recalculate through the same path as a manual edit — no second copy of the billing maths. Skips the source row, undated rows, standalone staff rows (blank studio) and **`admin_locked` rows** (approved work is not silently rewritten). Local-first like every other edit there, so **Cancel still reverts a mis-click**. `studio` and `location` travel together or they'd disagree. UI: a small `⇊` in the cell, shown **only on the row you just edited** (`lastEditedRowId`, set in `updateStRow`) — in every cell of a 30-day table that would have been ~120 buttons.

**v1.3.1 — fixes from Eli's device test of v1.3.0:**
- **"Changing the engineer name created duplicate rows" — my bug, in `saveStaffName`.** The 'all days' path set `eng_visible: true` on every same-role row, which **un-hid staff lines on rows whose `eng_visible` was false precisely because a standalone staff row already covered that day** — producing two identical staff lines per date. 'all' now targets only rows that ALREADY show a staff line ("what you can see is what changes") and **never touches `eng_visible`**; only an explicit single-row assignment may reveal a hidden line. The button also counted ROWS, so a 3-day booking advertised "all 4 engineer days" — it now counts **distinct dates**.
- **`<datalist>` replaced with a real tappable list.** On iOS Safari a datalist renders as a floating autofill-style bubble mid-screen with no visible dropdown, so the roster looked broken and you had to know to type. Now chips you tap, plus `autoComplete/autoCorrect/spellCheck` off to stop iOS offering "AutoFill Contact" over a name field. Free text still allowed.
- **Staff times now FOLLOW the session times** (Eli: "everytime there is a studio time line and an eng below it, that engineer is attached to that session… but it does need independent editing"). In `updateStRow`, changing `from_time`/`to_time` updates `eng_from_time`/`eng_to_time` **only when the staff time was empty or still equal to the session's previous time** — i.e. it was following along. A deliberately different staff time (engineer in early, out late) survives untouched. Note a staff sub-row is the SAME `studio_time_rows` record, so studio/room changes already applied to it; standalone staff rows have no room of their own by design.
- **`eng_hours` written back in the eng-charge block.** It recomputed the charge from the times but left the hours stale, so a saved row could carry hours that didn't match its own charge until the next reload re-derived them.

**v1.3.2 — bulk editing moved to Admin as an explicit Batch Edit (supersedes parts of v1.3.0/v1.3.1).** Eli, on reflection: *"I don't want the runner to have any apply to all. That should only be admin. Maybe there is a batch edit button on the admin, instead of the little down arrows."*

**Rationale (worth keeping):** a runner is recording what happened on their own shift, from a phone, on a shared device — an accidental "all days" there silently rewrites days they have no visibility into. Bulk change is a planning act, so it belongs on the planning surface. And a bulk edit reads better as a deliberate mode than as ~120 tiny fill-down arrows scattered through a 30-day table.

- **Runner:** `saveStaffName()` is single-row only; the all-days button, the role-scoped target set and the distinct-day count are gone. The sheet is now one **Save**.
- **Admin:** the per-cell `⇊` controls, `applyFieldToAllDays`, `fillDownTargetCount` and `lastEditedRowId` were all removed. Replaced by a **Batch Edit** panel opened from the Studio Time `SectionHeader` action.
- **Scope:** *All days* or *Date range* (two date inputs, not thirty checkboxes — the data is date-keyed and it matches how the change is described out loud: "the 5th through the 9th move to B").
- **Fields (Eli chose everything on the row):** room, start, end, rate + Day/Hr type, OT hours, OT rate, staff (1ST/2ND + roster name), session info. **Each has its own checkbox and unticked fields are never written**, so a blank input can't wipe a column by accident.
- **Targets:** dated STUDIO rows only. **Standalone staff rows are excluded** (no room/times/rate of their own — sweeping them up would silently rewrite someone's extra assistant) and **`admin_locked` rows are excluded**, with the footer reporting how many approved days it skipped rather than staying silent.
- **Staff semantics:** one row carries ONE staffer + role, so batch **sets** that line (overwriting whatever it held) rather than adding a second person. Both roles on one day needs a standalone staff row — that's an add, not an edit, so it's deliberately out of batch.
- Everything routes through **`updateStRow`**, so charge/OT/engineer totals recalculate through the same path as a manual edit — still no second copy of the billing maths. Local-first: nothing is written until the WO is saved, so **Cancel reverts the whole batch**.
- Footer states the outcome before you commit ("Will change 12 days · skipping 3 approved") and Apply is disabled until a field is ticked and the scope matches something.

**v1.4.0 — DEV tab (Feedback + PIN-gated Testing) + runner error checks.** Eli is bringing in a helper to test, so he needed a way to hand someone a structured checklist.

*Runner error checks (the standing-rule gap, now closed on the highest-risk page).* `dbResult` was imported into the runner WO page yesterday for one write; the rest were unchecked. Worst of them: **`handleSaveChanges` called `router.push` back to the studio hub unconditionally** — a save that failed on flaky studio wifi bounced the runner to the hub *believing their shift was recorded*, with nothing surfaced anywhere. It now collects every write's error (WO fields, studio-time rows, payment rows), reports once, and **stays on the page so the work is still on screen to retry**. Also wrapped: session notes, equipment condition + notes, the date reorder, and both needs-attention photo writes (which were `console.error` only — invisible on a phone; a failed photo write now restores the previous list rather than leaving the UI lying).

*DEV tab.* Nav item `Feedback` → **`DEV`** (route stays `/feedback` — renaming it would break bookmarks for no gain). The page is now a tab shell: **Feedback** (the existing board, moved verbatim into a `FeedbackBoard` component — Eli: *"it's great don't touch it"*) and **Testing**.

*Testing section — `components/dev/TestingSection.tsx`.* PIN `4321`, unlocked per browser session. **The PIN guards Testing only**, so feedback stays one tap away as before. It is explicitly a *soft* gate, not security — everything behind it is already readable by any signed-in staff under RLS, and there's a comment saying so, so the pattern doesn't spread to anything that matters.

**Key design decision: batch definitions live in CODE (`lib/testBatches.ts`), only verdicts in the database.** Because (a) a batch is authored in the same commit as the work it tests, so checklist and feature can't drift, (b) no migration and no SQL for Eli per batch, and (c) the wording isn't data — the human verdicts are. Migration `20260729120000_test_results.sql` is therefore ONE small table, unique on `(batch_id, item_id)`; Eli confirmed a **single tester**, so there's no per-user dimension and re-testing upserts. RLS: any authenticated staff read/insert/update (a helper must be able to record), delete owner/manager (reset a run without a tester wiping history).

Batches are **batches, not one growing list** (Eli), pairing with releases. First one seeded: `wo-runner-2026-07`, ~40 items across Runner access, Runner WO, Admin WO, CRM and Dashboard, each with a plain-English *what* and step-by-step *how* written for someone who has never seen the feature. UI: progress bar + Works/Broken/Not-tested counts, per-area filter with per-area progress, and per item a Works / Doesn't-work pair plus a note (kept on either verdict). Optimistic writes so tapping through 40 items doesn't wait on round trips, reverting via reload if the write fails. Realtime subscription on `test_results` so Eli watching on a laptop sees verdicts land live.

**Item ids are load-bearing** — they're half the results key, so renaming one after testing starts orphans its verdict. Noted in the file.

**v1.4.1 — batch cards + a floating tester panel (Eli's corrections to v1.4.0).**

*Miss worth recording:* v1.4.0 modelled the data as batches but **rendered them as one flat 38-item list**, so the model was right and the UI didn't express it. Bigger miss: the checklist lived on its own page. **A tester is holding a phone, reading step 12, trying to tap the thing step 12 describes** — a checklist on a separate screen means tabbing back and forth for every item, which is how items get skipped or mis-marked. Eli caught this by imagining the person; I had been designing the interface. Worth remembering as a failure mode: design the task, not the page.

- **Batch cards.** Testing now lands on a card per batch with live progress, a state pill (Not started / N tested / Done · all working / Done · N broken) and a pass-fail bar. Finished batches read as finished without being hidden — you may still want to see what failed. "Review results" gives a read-out grouped **failures first**, with every note and who recorded it. This also retires the deferred "active/done flag" idea: status is *derived* from results, not declared, so it can't go stale.
- **Floating tester panel** — `components/dev/TestingFloater.tsx`, mounted in the **`(main)` layout**. It survives navigation across the internal app, and is deliberately **NOT on `/runner`**: Eli clarified that runner testing is done on a **phone** with this checklist open on a computer, so a panel there would only cover the phone-first UI being tested. (My first pass mounted it at the ROOT for the opposite reason — wrong, and it would also have shown the panel on `/login`, `/register` and `/inquiry`.) Items now carry a **Phone / Desktop badge** (`deviceFor()` — explicit `device` field falling back to the area name) because the tester is working two screens at once and shouldn't have to infer which. The batch intro states the workflow. Shows **one item at a time** (a scrolling list inside a small window is worse than useless): what, how, Works/Broken, note, prev/next. **Draggable** with a clamped position so it can't be dragged off-screen — anticipated rather than discovered, since it will inevitably sit over the button being tested. Minimises to a pill; position and minimised state persist. Auto-advances on a verdict (the tester's hands are on the app, not the panel) and opens on the first untested item rather than the top. `zIndex` 100003 — above the Nav, below the welcome splash.
- **New `hooks/useTestResults.ts`** — the fetch/upsert/realtime/rollback path and `batchProgress` live in ONE place because the page and the panel both write verdicts. Two copies of a write path is the drift risk that's already bitten this codebase, and this one writes data a human can't reconstruct. Unlock state moved here too (sessionStorage + a window event), since the PIN is entered on the DEV page but the panel is mounted in a different part of the tree.
- **Correction to a claim I made:** the PIN was never re-entered per visit — `sessionStorage` already persisted it for the browser session. The tab-switching complaint was real; the re-entry part wasn't.

- **Card layout considered and DROPPED (decision, don't revisit without new information).** I proposed stacked per-day cards; Eli's constraint killed it: **a WO can run 30 days**, and 30 cards is 30 screens of scrolling to answer "what time are we Thursday". Runners also need *every* column — they read this out to clients on the phone — so cards' legibility-for-density trade is the wrong way round here. His own diagnosis ("I think it's just colors or type size") was correct and is what v1.3.0 did instead. A later idea — card for the single-day view, toggling into the table — was also **deferred**: after the readability pass the today-only view is 1–3 legible rows, so the card's remaining gain (no horizontal scroll) shrank a lot, and a second editable layout over the same rows is real drift risk in a 1,387-line file. If revisited, the precondition is extracting the inline per-row maths (~25 lines of hours/OT/total/eng-charge) and the field controls first, so both layouts consume one source — otherwise it duplicates billing maths, the exact audit defect class.
...
### July 29, 2026 (cont.) — v1.4.2 / v1.4.3: testing tooling fixes, DEV restructure, docs for handoff

**Testing tooling fixes (all from Eli's live use, all mine):**
- **Duplicate realtime channels → the error screen.** `useTestResults` named its channel `test-results-${batchId}`, but the hook mounts in **three** places at once (a card per batch, the review view, the floating panel). The live `app_errors` entry gave the exact mechanism: `cannot add 'postgres_changes' callbacks for realtime:test-results-wo-runner-2026-07 after 'subscribe()'` — note **no instance suffix**. supabase-js hands back the *same already-subscribed channel object* for a duplicate name, so the second mount's `.on()` throws. Fixed with a module-level sequence appended per mount. **This is the error-monitoring pipeline paying for itself** — Phase 0 built it in July and this is the first time a production stack trace shortcut a diagnosis.
- Removed two `!` assertions on `TEST_BATCHES.find(...)` — a stale batch id in `sessionStorage` would have taken the page down.
- **"Continue" always opened at item 1.** The positioning effect ran on batch-id change alone, when `results` was still empty because the fetch hadn't resolved, so "first untested" always computed as item 1. Now waits on `loading` and fires once per batch via a ref.
- **Header counter read as stuck.** It showed `tested/total` (verdicts recorded), which looks like a position that should move with Prev/Next. Now shows both, labelled: `Item N of 38` / `N done`. *Lesson: label counters — conflating two numbers reads as a bug even when each is correct.*
- **Notes flow reworked (Eli's spec):** notes moved **above** the verdict and always visible; the note saves **with** the verdict. Previously "Save note" defaulted `status` to `'fail'`, writing a verdict nobody chose. Prev/Next at the bottom, **Next blocked until a verdict exists**, auto-advance removed so a tester can go back and change an answer.
- **Runner all-days removed** (Eli, on reflection): bulk edits belong on the planning surface, not on a shared phone where an accidental "all days" rewrites days the runner can't see. Replaced on admin by the Batch Edit panel.

**DEV page restructure.** Errors moved from Admin → DEV and gated to **Eli's accounts only** (hides the surface; `app_errors` RLS still allows owner/manager, and the export depends on that). DEV rebuilt with a **left sidebar mirroring Admin** so the two internal tool pages navigate identically.

**Failure export — closing the loop.** Claude has no database access, so a captured error or a tester's note was only useful if Eli retyped or screenshotted it, which is where the URL, `meta.source` and exact message get dropped. Added **Copy for Claude** (Admin/DEV → Errors) and **Copy failures + notes** (batch review). Also filtered provably-external noise out of the error sink (`window.webkit.messageHandlers` probes, opaque `Script error.`, benign `ResizeObserver loop`) — an error log nobody trusts is one nobody reads.

**Documentation for handoff (Eli: "just to have in case I die").**
- **`docs/CHANGELOG.md`** — developer-facing, per **version**, newest first. Every entry states **migrations**, **watch-outs** and files touched. PROJECT_LOG can't answer "what changed in vX" because it's organised by session; this can.
- **`docs/ONBOARDING.md`** — the cold-start entry point: what the app is, how to run it, the four decisions that will confuse you, the landmines, and **an operational access map** (Supabase, Vercel, GitHub, domain via Pixelgate, backup service account). No secrets. **Flagged in the doc: every one of those systems is reachable only via a locked note in Eli's Mac Notes — that single point of failure matters more than any documentation, and a second copy in a shared password manager is the highest-value action on the page.**
- **SOP version history gained collapsible dev notes** per version, which **mirror the CHANGELOG entry** (Eli: "it should just mirror what lives in the folders, not write new ones"). Copy the bullets, don't paraphrase; migrations/files stay in the repo copy. Codified in CLAUDE.md.
- The wrap-up ritual in CLAUDE.md is now **five** items (PROJECT_LOG · CHANGELOG · Tech-Stack · SOP VERSIONS · testBatches), with the five-docs-five-audiences rule spelled out so nobody merges them.

**Two test batches for one day, deliberately.** `wo-runner-2026-07` was written mid-session and covers v1.1.0–v1.3.2; v1.4.x shipped after it. Rather than grow the first batch (which the rules forbid) a second batch covers the DEV tooling. This is the mid-session-supersede problem the per-session rule exists for, showing up on day one.

**Added to the future list: convert `public/sop.html` into a real React page.** It is standalone HTML in an `<iframe>` with its own `:root` colours and its own fonts — **no `data-theme` awareness at all**, so with the app defaulting to LIGHT mode, staff opening SOP get a black page. It also still uses DM Mono as its body font (the app moved to Inter in July), can't share components or CSS variables, and can't read the changelog — which is exactly why the dev notes have to be hand-mirrored. Converting is a ~900-line file with bespoke CSS: a real project, not a tidy-up, and explicitly **not** to be started mid-test-run. A cheap interim exists (pass the theme into the iframe and map its variables) if the dark page becomes annoying first.

---

### July 29, 2026 (cont.) — SECURITY INCIDENT: distributed brute force on PIN login

**What happened.** Eli was looking at the Vercel request log for an unrelated reason and noticed a wall of `POST /api/auth/pin` — mostly `401`, occasional `429` — at roughly one per second. `pin_login_attempts` showed **~50 distinct IPs**. That spread is deliberate: the per-IP lockout was never tripped hard because no single IP had to carry the load. This was a distributed brute force, not a staff member fumbling a PIN and not a stuck PWA.

**Why it was worth someone's time — the arithmetic.** `verify_staff_pin(p_pin)` takes **only the PIN**. There is no username, no email, no account selector: the PIN alone is the whole credential, matched against every staff row at once. With a 4-digit PIN (10,000 combinations) and ~10 provisioned staff accounts, **any random guess has roughly a 1-in-1,000 chance of landing on a real account**. A few thousand attempts is a likely hit. That is a completely different risk profile from "guess one named person's PIN", and it is the thing to understand before touching this code.

**The bug that made it cheap.** The lockout did this:

```ts
fail_count: willLock ? 0 : nextCount   // ← reset to 0 on lockout
```

Five fails → 30-second lock → **counter back to zero**. So every IP got 5 fresh attempts every 30 seconds, forever, and the escalation the code appeared to have was unreachable. ~600 attempts/hour/IP, ×50 IPs.

**What shipped tonight.**
- Counter **never** resets on lockout; it only climbs. Lockout escalates `30s → 2m → 10m → 60m`.
- **Capped at 60 minutes deliberately.** Staff at one studio share that location's public IP, so an unbounded or 24h lock would let one runner's mistype take a whole room offline mid-session with no way back in — worse than the attack. This cap is a real trade-off, not an oversight: don't "harden" it without solving the shared-IP problem first.
- Forgiveness is now a **6-hour quiet period since the last failure** (`DECAY_MS`), replacing reset-on-unlock. A daytime mistype is forgotten by the evening session; a paced attacker gets ~4 attempts per IP per 6 hours instead of 600/hour.
- New append-only `pin_login_failures` (ip, user_agent, outcome, created_at). `pin_login_attempts` holds one row per IP and is **deleted on success**, so it can never answer "what happened last night" — hence a separate history. The **attempted PIN is deliberately not stored**: it would put near-misses against live credentials, and typo'd real PINs, in plaintext.
- Login screen humanises the countdown (`3600s` → `60m`).
- **Vercel Bot Protection set to Challenge** (dashboard, no deploy). Vercel advises starting at "Log"; that advice is for calm evaluation, and we were mid-attack. Challenge blocks non-browser traffic while real browsers — including the installed PWA — pass untouched. **Watch the two Vercel Crons** (`auto-demote` 09:00, `reset-needs-action` 15:00): they are also non-browser traffic.

**Idea rejected: a global rate limit with email/password as the fallback.** Proposed, then withdrawn on discovering *why* it doesn't work here — staff Supabase passwords are random 32-character strings generated by `scripts/set-staff-passwords.mjs` that **nobody knows**. The PIN is the only way in for the whole team, so anything that throttles PIN login globally locks out all staff with no recovery path. Any future rate-limiting design has to reckon with that.

**The decisive fact, which arrived late: the app is not live yet.** Only a couple of real users (a night-time manager working tonight). That reverses the cost/benefit entirely — aggressive lockdown is nearly free, and is a better answer than tuning throttles around a weak credential. **This should have been asked before designing mitigations.** The lesson generalises: "how many real users does this have right now" changes which security trade-offs are even worth making, and it is cheap to ask.

**Therefore, tonight's plan (Eli's call):** revoke every staff PIN except the night manager's; revoke existing auth sessions (a deleted PIN does **not** invalidate an already-issued token — a session an attacker obtained earlier keeps working until its refresh token dies); Eli sets a real password for himself first so there is a non-PIN way in. Deleting `staff_pins` rows is the right kill switch precisely *because* everyone re-enrols tomorrow — no migration, no `is_active` column, no new code surface.

**Deliberately NOT built: a self-serve "enter your email and pick a password" screen.** An unauthenticated form where someone types a staff email and sets a password lets anyone who knows an address claim the account. Enrolment must be invite-based — and Supabase's `resetPasswordForEmail` plus the existing `/reset-password` page already does exactly this, so the safe version needs **no new code**.

**Still open (see Future Considerations):** 4-digit PINs verified without any identity remain the structural weakness. Two candidate fixes, in order of preference: **(1)** 6-digit PINs — 10,000 → 1,000,000 combinations, and mostly a conversation with staff rather than code (`/^\d{4}$/` in the route, the numpad length, re-seeding hashes); **(2)** an account selector before the PIN, which allows locking the **account** instead of the IP and is therefore immune to IP rotation — at the cost of one extra tap and of walking back the "one screen, no choices" login Eli asked for.

**Two operational landmines found while executing the lockdown, both worth knowing before onboarding the team.**

**1. Supabase's built-in email sender caps at a couple of messages an hour.** It is a testing convenience, not a mail service. Eli's own reset attempts (the first of which went to `localhost` — see below) exhausted the quota, and the next request failed while a manager sat at the desk about to lose his PIN. `scripts/set-one-password.mjs` was written as the manual override: set a known password directly via the admin API, say it out loud, let them change it themselves at `/reset-password` (which is authenticated and sends no email, so the cap never applies). **Pointing Supabase Auth at Resend SMTP is now the first task tomorrow and blocks the invites** — nine people onboarding at once will hit this at the third person.

**2. Password reset emails were going to `http://localhost:3000`.** Not a code bug: `login/page.tsx` correctly hardcodes `redirectTo: 'https://prsflow.paramountrecording.com/reset-password'`. Supabase validates `redirectTo` against the **Redirect URLs allowlist** and, when it isn't on the list, **silently falls back to the project's Site URL** — which was still `localhost:3000` from initial setup. Fixed in the dashboard (Authentication → URL Configuration): Site URL set to production, `https://prsflow.paramountrecording.com/**` and `http://localhost:3000/**` added to Redirect URLs. This would have hit every staff member tomorrow.

**A third, smaller trap, now guarded in code:** macOS substitutes typed quotes with typographic ones, and bash does not treat `‘ ’` as quoting characters — it passes them through literally. The first real run of `set-one-password.mjs` therefore set a password *including* the curly quote marks, and the login failed with "invalid email or password" while everything in `auth.users` looked perfectly healthy (emails matched, all confirmed, none banned). The script now refuses smart quotes outright rather than stripping them, because silently fixing it would hide the same mistake next time.
