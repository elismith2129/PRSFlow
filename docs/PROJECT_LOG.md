...
- Workflow note: the Cowork sandbox can't unlink files on the mount, so every git write strands a `.git/*.lock` file. Rule for these sessions: **Claude edits files only; Eli runs all git commands** (`rm -f .git/*.lock && git add -A && git commit … && git push`).
