...
**Deferred to next session:** Step 8 (delete BookingForm — legacy blocks still ride on it), Step 9 (retire `work_orders.booking_id` — note: the create RPC's `on conflict (booking_id)` makes this migration depend on re-keying idempotency first), UI redo Phase 2 kickoff. Sequenced after the atomic RPCs are proven in production — one risky change at a time.
