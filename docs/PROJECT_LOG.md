...
- **Two separate manifests + icon sets: main app vs. runner hub.** `public/manifest.json` (`name: "PRSFlow"`, `start_url: "/"`) and `public/runner-manifest.json` (`name: "PRSFlow Runner"`, `short_name: "Runner"`, `start_url: "/runner"`) are distinct so studio runners can install the **Runner hub as its own home-screen app** that launches straight into `/runner`, with a visually distinct icon (the runner icon adds a teal `RUNNER` badge under the PRS FLOW wordmark). Both use `display: standalone`, `background_color`/`theme_color` `#0d0f14`, and three icons (192, 512, 512-maskable).
...
- **`public/manifest.json`** (main) — `name "PRSFlow"`, `start_url "/"`, `display standalone`, `background_color`/`theme_color` `#0d0f14`, icons 192/512/512-maskable.
- **`public/runner-manifest.json`** (runner) — `name "PRSFlow Runner"`, `short_name "Runner"`, `start_url "/runner"`, same display/colors, runner icons 192/512/512-maskable.
