...
- **Add-task photo "not saving" — under investigation (resume June 26, 2026).** Symptom: files reach storage (`checklist-photos/dashboard-tasks/`) but some recent `dashboard_tasks` rows have `photo_url = NULL`. Anon-key probes proved the save path is healthy: the `photo_url` column exists, an insert with a `photo_url` value reads back correctly, and `uploadPhoto` (upload + `getPublicUrl`) returns a valid URL. The newest storage file is from ~June 17; recent NULL rows are test tasks ("fernando test 1/2", "asst manage test 1") that appear to have been saved **without a photo attached**. Added a thumbnail preview (`URL.createObjectURL`, shown on file select) + `.select()` insert-error logging (`056aa79`) so attachment state is obvious and any real failure surfaces in the console. Next: reproduce with the preview, capture any console error, and check the bucket's anon INSERT policy + filename sanitization (`uploadPhoto` path only strips whitespace, not parentheses/commas/HEIC oddities).
