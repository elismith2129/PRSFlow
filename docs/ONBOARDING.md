...
- **The Vercel Firewall has a custom Bypass rule named "Allow PWA icons" — deleting it silently breaks iPhone home-screen icons.** (Aug 22, 2026.) Vercel's Bot Protection challenges "non-browser sources", and iOS fetches Add-to-Home-Screen icons with a background system service — a non-browser source. Challenged fetch → iOS 26 shows a letter tile ("P") instead of the app icon, with **no error logged anywhere**: Safari loads the PNG fine, Android works, desktop works, only the iPhone home-screen tile breaks. The rule bypasses challenges for request paths starting `/apple-touch-icon`, `/icon-`, `/runner-`, `/manifest.json`, `/favicon` — all static public assets, so the bypass protects nothing sensitive. **It lives in the Vercel dashboard (project → Firewall), NOT in the repo** — a redeploy can't restore it and `vercel.json` doesn't mention it. Diagnosing this from scratch took a full day; see PROJECT_LOG Aug 22 for the elimination path.
